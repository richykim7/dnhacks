...
- Knowledge: a full-height graph workspace with a compact toolbar and no permanent side panel. Every matching claim and endpoint is retrieved with `complete=1`, without the legacy row cap. The Minimum connected claims slider defaults to 2, hiding single-claim entities and their incident edges; 3 also hides double-claim entities, and 1 restores all nodes. Its range reaches the largest claim degree in the loaded graph, with an adjacent number input for precise values. Counts use distinct claims in one pass over the loaded graph, with self-loops counted once. Visible counts are separate from loaded totals; changes preserve layout and do not reload or mutate data. Counts show loaded versus matching/collection totals. Retrieval is ordered by claim ID, independent of status; the initial camera centers the entity with the most distinct connected neighbors. Fit all shows every component and Connected region restores the starting camera. Optional status/search filters remain explicit. Labels use collision-aware zoom thresholds and hover/selection reveal; low-zoom edges use restrained opacity and reveal direction markers on zoom or selection. Self-loops and parallel relationships retain separate paths. Offscreen SVG elements are culled from the viewport while all nodes and edges remain loaded; fit-all uses declared node geometry, including unmounted nodes. `?sceneReview=1` exposes read-only graph getters for the opt-in capture check. The source inspector opens on entity/claim selection or explicit Browse, scrolls independently, and closes with Escape, its close button or the canvas. No review, combining or approval controls are mounted. The literature graph as stored, with nothing invented on the way to the screen, laid out as a force-directed graph (d3-force, deterministic, run to rest before first paint) rather than layered ranks. Each entity is a shape for its kind, sized by how many claims touch it in view, with its label beneath and its ontology identifier (linked through Bioregistry) in the inspector and on the selected node; edges carry the claim's sign in biological notation (arrowhead enables, bar represses, open dot for an unsigned predicate), line weight by distinct source count, and a dashed amber stroke for corpus-disputed claims. Claims between the same two distinct entities fan apart so a dispute is visible as two edges. Self-loop edges are hidden from the canvas and visible-claim list/count; their stored claims and corpus totals remain unchanged. Search runs in the database over the whole collection; status, sign, entity kind and relation class filters are closed-vocabulary and show real counts. A summary strip states claims, entities, evidence records, papers (full text), reported experiments and engine tests with the graph file's modification time. Selecting a claim shows its status with what the word may mean (a source count, never approval or proof), the subject → predicate → object spine with entity state and variant, aspect, sources, first mention, mechanism and dispute kind, the other claims answering the same question, every engine test on the claim (predicted versus observed sign, effect, p, verifier outcome, reviewer decision and note, novelty), then each evidence record: paper with DOI/PubMed/PMC links, full-text and licence status, section, quotation, evidence type, study type, attribution and certainty with checker agreement, the source's own wording, what it cites, the experiment it reports, and biological context with provenance. Identifiers and extraction metadata sit in disclosures. Missing values say so. Review controls are not shown; backend decision and promotion APIs remain available.
...
- `GET /api/kg?source=&complete=1&limit=&status=&polarity=&kind=&relation_class=&predicate=&q=` returns nodes with `kind` and `curie`, edges with `claim_id`, `abstract_key`, `polarity`, `relation_class`, `mechanism`, `first_year`, `n_sources`, `status`, `dispute_kind` and a `tested` overlay counted from `engine_tests`, plus `complete`, `loaded_entities`, `matched`, `total_claims`, `status_counts`, `facets`, `summary` and `as_of`. `complete=1` omits the SQL row limit; legacy bounded callers retain their limit. All responses use status-neutral claim-ID order. Entity identities come from claim endpoints; this schema has no standalone isolated entity table. Filters are equality tests on closed vocabularies (unknown values are ignored); `q` is a case-insensitive substring match run in the database over labels, identifiers, predicate, aspect and mechanism. `GET /api/kg?source=<collection>&claim=<claim_id>` reads the exact collection's claim (with the entity-form columns the view drops), up to 100 evidence records each with contexts, citations, source paper and reported experiment, `evidence_total`, the other claims sharing its `abstract_key`, and its engine tests. Claim inspection requires an explicit source; unknown sources/claims return 404. Nothing here extracts papers, runs models or changes the graph. Older graph responses without IDs retain relationship browsing but cannot open source details.
...
`KNOWLEDGE_REVIEW_SNAPSHOT=/tmp/knowledge-review.json`: a read-only `/api/kg?complete=1`
...
checks all unique graph IDs against collection totals, status filters, source scrolling,
and default/fit-all camera bounds, and captures both views at 1920×1080. Without a snapshot the optional case explicitly skips.
