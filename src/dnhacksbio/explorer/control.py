...
def validate_report(value):
    ...
    if len(encoded(value)) > 24000:
        raise ValueError("Report exceeds 24000 characters")
    return value


def validate_decision(value, max_actions=18):
    ...
    if value["action"] == "fork":
        ...
        if not isinstance(bs, list) or not 2 <= len(bs) <= 3:
...
class ControlStore:
    ...
    def start(self, run_id, allowance, total_nodes=72):
