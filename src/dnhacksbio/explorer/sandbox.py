...
def _build_run_argv(name: str, image: str, jobdir: str, data_dir: str | None, timeout: int,
    ...
    argv = ["run", "--rm", "--name", name,
            "--network", network,
            *(["--memory", memory] if memory is not None else []), "--cpus", str(cpus), "--pids-limit", "512",
            "--user", f"{uid}:{gid}",
...
def run_code(code: str, *, image: str = IMAGE, timeout: int | None = 600, memory: str | None = "8g", cpus=4,
