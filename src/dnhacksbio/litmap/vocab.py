...
CATEGORIES: dict[str, dict] = {
    ...
    "cellular_component": {"owner": "GO", "ns": ("GO",), "path": "process",
                           "when": "a named organelle or molecular complex in GO, e.g. MHC class II protein complex"},
    "cellular_phenotype": {"owner": "local cellular phenotype registry", "ns": ("LOCALPHENO",), "path": "entity",
                           "when": "a defined cellular outcome absent from the standard owners, e.g. centrosome amplification or clustering; use only registry entries"},
    "local_disease": {"owner": "local disease subtype registry", "ns": ("LOCALDISEASE",), "path": "entity",
                      "when": "a reviewed disease subtype absent from MONDO; use only registry entries, never collapse a specific subtype into its broader disease"},
    "experimental_reagent": {"owner": "local reagent registry", "ns": ("LOCALREAGENT",), "path": "entity",
                             "when": "a specifically defined experimental reagent absent from ChEBI; use only registry entries"},
    "cell_population": {"owner": "local cell population registry", "ns": ("LOCALCELL",), "path": "entity",
                        "when": "a defined cell population absent from CL, e.g. inflammatory, myofibroblastic or antigen-presenting cancer-associated fibroblasts; use only registry entries"},
