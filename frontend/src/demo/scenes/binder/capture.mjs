...
const server = await createServer({ root, configFile: false, cacheDir: '/tmp/binder-cinematic-vite', esbuild: { jsx: 'automatic' }, optimizeDeps: { entries: ['src/demo/scenes/binder/preview.html'] }, server: { host: '127.0.0.1', port: 0 } });
let browser;
try {
  await server.listen();
  ...
  await page.screenshot({ path: path.join(output, 'binder-1920x1080.png') });
  await page.evaluate(() => window.dispatchEvent(new CustomEvent('binder-time', { detail: 0 })));
  await page.waitForTimeout(500);
  await page.screenshot({ path: path.join(output, 'binder-establish-1920x1080.png') });
  await page.evaluate(() => window.dispatchEvent(new CustomEvent('binder-time', { detail: 15 })));
  await page.waitForTimeout(500);
  const repeat = await page.screenshot();
  const { readFile } = await import('node:fs/promises');
  const deterministic = repeat.equals(await readFile(path.join(output, 'binder-1920x1080.png')));
  await context.close();
  if (!process.argv.includes('--still-only')) {
    ...
    await rename(await video.path(), path.join(output, 'binder-preview.webm'));
  }
  await writeFile(path.join(output, 'capture.json'), JSON.stringify({ renderer: 'Chromium SwiftShader', viewport: [1920, 1080], stillTime: 15, repeatedTimePixelIdentical: deterministic, errors, provenance: 'Illustrative seeded geometry; not a scientific result' }, null, 2) + '\n');
