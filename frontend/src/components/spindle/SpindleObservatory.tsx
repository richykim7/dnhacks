...
import SpindleScene from "./SpindleScene";
import {
  validateBundle,
  type SpindleBundle,
  type SpindleRecipe,
...
import "./spindle.css";
export default function SpindleObservatory({ url }: { url: string }) {
  ...
  useEffect(() => {
    ...
    void fetch(url, { signal: abort.signal })
      .then(async (r) => {
        ...
        return validateBundle(await r.json());
      })
      .then((b) => {
        setRecipe({
          frame: 0,
