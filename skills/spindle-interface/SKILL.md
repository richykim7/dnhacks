...
6. `export_scene_movie`: args `{recipe_sha256,frames:[0,1,2],fps:10,viewport:[1600,1000]}`. Choose 2–120 increasing saved frame indices; the camera is fixed, comparison uses nearest saved physical time, and no geometry is interpolated. It publishes a scoped WebM plus physical/presentation timeline and render measurements. At most four movies per experiment; rendering is bounded to five minutes and 8 MiB. Dense fields may require fewer frames. Read the recorded timing scope: GPU draw timing is not full interactive frame rate.

Operator setup must provide a pinned native build and reachable local scene server. Movie export additionally requires an operator-set absolute `SPINDLE_FFMPEG_BIN`; executable hash and version are recorded. An unavailable build or capture is an explicit operational failure, never evidence against clustering. Existing rigor and human promotion gates remain in force.
